...
func _draw() -> void:
	draw_rect(Rect2(0, 0, size.x, size.y), Color("06040c"))
	...
	for i in range(90):
		...
		draw_rect(Rect2(rng.randf() * size.x, rng.randf() * size.y, 2, 2), col)
	var scroll := t * 34.0
	for i in range(LINES.size()):
		var y := size.y + 40.0 + i * 44.0 - scroll
		if y < -40.0 or y > size.y + 40.0:
			continue
		var line := String(LINES[i])
		var is_heading := line.begins_with("#")
		draw_string(font, Vector2(0, y), line.trim_prefix("#"), HORIZONTAL_ALIGNMENT_CENTER, size.x,
